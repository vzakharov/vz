...
async function matchesFilter(object, filter, caller, context, parentKey) {
    if (!filter)
        return true
    if (isArray(object) && !filter.length) {
        ...
        for (let item of items) {
            if (!(await matchesFilter(item, filter, caller, context, parentKey)))
                return false
            continue
        }
    } else {
        for (let key in filter) {
            ...
            if ( isUndefined(item) ) { //&& isFunction(caller.fetch)) {
                let { _fetch } = caller.subSchema[key]

                // Todo: combine 👇 with the one in AsyncIterable (same code duplicated)
                if (isFunction(_fetch)) {
                    item = await _fetch(context)
                } else if (parentKey) {
                    let { _fetchChildren } = caller.subSchema[parentKey]
                    if (isFunction(_fetchChildren)) {
                        item = (
                            await _fetchChildren(context)
                        )[key]
                    } else {
                        return false
                    }
            ...
            if (
                ...
                let match = await matchesFilter(item, subFilter, caller, context, key)
                if (match)
                    continue
            }
            return false
